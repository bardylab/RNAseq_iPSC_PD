...
datadir <- paste0(maindir, "/6_salmon_output_nospikes/DAN")
...
save(txi.salmon, file = paste0(resultsdir, "/Shong_DAN_salmon_txim.RData"))

## Retrieve and export the counts table
salmon.counts <- txi.salmon$counts
salmon.counts <- as.data.frame(salmon.counts)
write.csv(salmon.counts, file = paste0(resultsdir, "/Shong_DAN_salmon_counts_unfiltered.csv"), quote = FALSE, row.names = TRUE)
save(salmon.counts, file = paste0(resultsdir, "/Shong_DAN_salmon_counts_unfiltered.RData"))

## Retrieve and export the TPM table
salmon.TPM <- txi.salmon$abundance
salmon.TPM <- as.data.frame(salmon.TPM)
write.csv(salmon.TPM, file = paste0(resultsdir, "/Shong_DAN_salmon_TPM_unfiltered.csv"), quote = FALSE, row.names = TRUE)
save(salmon.TPM, file = paste0(resultsdir, "/Shong_DAN_salmon_TPM_unfiltered.RData"))
